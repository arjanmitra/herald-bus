...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!sessionId) {
            console.log('[history] missing session cookie');
            return NextResponse.json({ error: 'Not authenticated' }, { status: 401 });
        }

        const session = await findSessionById(sessionId);
        if (!session) {
            console.log('[history] invalid or expired session', sessionId);
            return NextResponse.json({ error: 'Invalid session' }, { status: 401 });
        }

        const history = await getUserUploadHistory(session.user_id);
        console.log(`[history] fetched ${history.length} rows for user ${session.user_id}`);
        ...
        if (list.length === 0) {
            console.log('[history] empty list returned');
        }
